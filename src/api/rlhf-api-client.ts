...
export class RlhfApiClient extends BaseApiClient {
   ...
   */
  async submit(
    body: RlhfSubmissionRequest,
    idempotencyKey?: string,
    opts?: ApiRequestOptions,
  ): Promise<ApiResponse<RlhfProxyResponse>> {
    return this.post<RlhfProxyResponse>(
  ...
  async grade(
    ...
    body: RlhfGradeRequest,
    idempotencyKey?: string,
    opts?: ApiRequestOptions,
  ): Promise<ApiResponse<RlhfProxyResponse>> {
    return this.post<RlhfProxyResponse>(
      `/api/v1/rlhf/grades/${encodeURIComponent(String(courseId))}/${encodeURIComponent(String(assignmentId))}`,
      body,
      withIdempotency(idempotencyKey, opts),
    );
  }
  ...
  async getRubric(
    ...
    return this.get<RlhfProxyResponse>(
      `/api/v1/rlhf/rubrics/${encodeURIComponent(String(questionId))}`,
    );
  }
}
