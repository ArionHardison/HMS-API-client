 ...
 */
export function assertSecureBaseURL(baseURL: string | undefined | null): void {
  if (!baseURL) return; // '' / undefined => relative / same-origin
  let url: URL;
  try {
    url = new URL(baseURL);
  } catch {
  ...
  if (url.protocol === 'http:' && !isLocalHost(url.hostname)) {
    throw new Error(
      `[wizard-api-client] Refusing to use insecure baseURL "${baseURL}": ` +
