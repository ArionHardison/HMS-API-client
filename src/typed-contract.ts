...
export type RequestBody<E extends OperationId> = Op<E> extends {
  ...
  ? B
  : Op<E> extends { requestBody?: { content: { 'application/json': infer B } } }
    ? B
    : never;
...
export type PathParams<E extends OperationId> = Op<E> extends {
  ...
  ? P
  : Op<E> extends { parameters?: { path?: infer P } }
    ? (IsNever<Exclude<P, undefined>> extends true ? never : Exclude<P, undefined>)
    : never;
...
export type QueryParams<E extends OperationId> = Op<E> extends {
  ...
  ? Q
  : Op<E> extends { parameters?: { query?: infer Q } }
    ? (IsNever<Exclude<Q, undefined>> extends true ? never : Exclude<Q, undefined>)
    : never;

// -----------------------------------------------------------------------------
// Request<E> — the composite input type
// -----------------------------------------------------------------------------
...
// Query params are always optional in the generated spec (all `query?`), so
// callers may omit `query` entirely.
 ...
 */
export type Request<E extends OperationId> = Simplify<
  BodyPart<E> & PathPart<E> & QueryPart<E>
>;
 ...
 */
export type HasNoRequiredInput<E extends OperationId> = {} extends Request<E>
  ? true
  : false;

// -----------------------------------------------------------------------------
// Response<E> — the 2xx JSON payload
// -----------------------------------------------------------------------------
...
export type Response<E extends OperationId> = Op<E> extends {
  ...
  ? {
      [S in Extract<keyof R, SuccessStatus>]: R[S] extends {
        ...
        ? C
        : never;
    }[Extract<keyof R, SuccessStatus>] extends infer Payload
    ? IsNever<Payload> extends true
      ? unknown
      : Payload
    : unknown
  : unknown;
