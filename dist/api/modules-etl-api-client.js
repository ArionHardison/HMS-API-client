...
exports.ETLModuleApiClient = void 0;
/**
 ...
 */
class ETLModuleApiClient extends api_client_1.BaseApiClient {
    // ---------------------------------------------------------------------------
    // Protocol integration (unversioned)
    // ---------------------------------------------------------------------------
    /** GET `/api/protocol/etl/all`. (`etl.protocol.all`) */
    listProtocolEtl(opts) {
        return this.get('/api/protocol/etl/all', undefined, opts);
    }
    // ---------------------------------------------------------------------------
     ...
     * (`etl.status`)
     */
    getStatus(pipelineId, opts) {
        const id = encodeURIComponent(String(pipelineId));
        return this.get(`/api/v1/etl/status/${id}`, undefined, opts);
    }
    // ---------------------------------------------------------------------------
    // Discovery (versioned)
    // ---------------------------------------------------------------------------
    ...
    components(opts) {
        return this.get('/api/v1/etl/components', undefined, opts);
    }
}
exports.ETLModuleApiClient = ETLModuleApiClient;
//# sourceMappingURL=modules-etl-api-client.js.map