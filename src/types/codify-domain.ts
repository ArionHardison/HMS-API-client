...
export interface DealTemplatePipelineStep {
  step: number;
  actor: string;            // role name OR system abbr OR 'hitl'
  action: string;
  inputs?: string[];
  outputs?: string[];
...
export interface DealTemplateFinancialModel {
  ...
  breakdown?: Array<{ onet_code: string; hours: number; cost: number }>;
}
...
export interface AgentComment {
  ...
  author_agent_id: string | null;     // UUID, when an agt LangGraph node authored
...
// ─── Domain agent profile (bulk endpoint) ─────────────────────────────
